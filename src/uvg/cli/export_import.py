...
def export(output: str | None) -> None:
    ...
    if not runtime_dir.exists():
        click.echo("Error: No runtime found. Run 'uvg sync' first.", err=True)
        raise SystemExit(1)

    lockfile = project_dir / "uvg.lock"
    if not lockfile.exists():
        click.echo("Error: No uvg.lock found.", err=True)
        raise SystemExit(1)
...
def import_runtime(tarball: str, force: bool) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        extracted_lockfile = tmpdir_path / "uvg.lock"
        if extracted_lockfile.exists():
            project_lockfile = project_dir / "uvg.lock"
            if project_lockfile.exists() and not force:
                click.echo("Warning: uvg.lock already exists, skipping", err=True)
            else:
