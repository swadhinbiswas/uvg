"""GVX CLI - Global package manager using UV's cache.
...
from gvx import __version__
from gvx.cli.export_import import export, import_runtime
from gvx.cli.workspace import workspace
...
@click.group()
@click.version_option(version=__version__, prog_name="gvx")
def main() -> None:
    """GVX - Global package manager with UV's speed."""
    pass
...
def init(python_version: str | None) -> None:
    """Initialize GVX in current directory."""
    ...
    lockfile_path = project_dir / "gvx.lock"
    if lockfile_path.exists():
        click.echo(f"gvx.lock already exists in {project_dir}")
        return

    with open(lockfile_path, "w") as f:
        json.dump(lockfile, f, indent=2)

    click.echo(f"Initialized GVX in {project_dir}")
...
def add(requirements: tuple[str, ...], python_version: str | None) -> None:
    ...
    if not lockfile_path.exists():
        click.echo("Error: gvx.lock not found. Run 'gvx init' first.")
        sys.exit(1)
    ...
    click.echo(f"Updated gvx.lock with {len(packages)} package(s)")
...
def sync(python_version: str | None) -> None:
    ...
    if missing_packages:
        ...
        click.echo("Try running: gvx add <package> to re-download")
...
def run(command: tuple[str, ...], python_version: str | None) -> None:
    ...
    if not runtime_dir.exists():
        click.echo("Error: Runtime not found. Run 'gvx sync' first.")
        sys.exit(1)
...
@main.command()
def info() -> None:
    """Show GVX information."""
    uv_cache = UVCache()
    python_version = f"{sys.version_info.major}.{sys.version_info.minor}"
    project_dir = Path.cwd()
    lockfile_path = project_dir / "gvx.lock"

    click.echo(f"GVX version: {__version__}")
    click.echo(f"Python version: {python_version}")
    ...
    # Show project info if in a GVX project
    if lockfile_path.exists():
        with open(lockfile_path) as f:
            lockfile = json.load(f)
...
def list_python() -> None:
    ...
    # Show current project version if in a GVX project
    project_dir = Path.cwd()
    lockfile_path = project_dir / "gvx.lock"
    if lockfile_path.exists():
        with open(lockfile_path) as f:
